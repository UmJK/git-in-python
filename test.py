...
if argument == "init":
    ...
    fo = open(".gitpy/dump/dump.txt", "rb+")
    repo = pickle.load(fo)
    if specific:
        if specific == "-A" or specific == "-a":
            fo = open(".gitpy/dump/addinfo.txt", "w+")
    ...
    try:
        ...
        if change:
            ...
            fo = open(".gitpy/dump/addinfo.txt", 'a').close()
        else:
        ...
        try:
            ...
            for i in files_in_cwd:
                if not os.path.isdir(os.getcwd()+ "/" + i) and i!="test.py":
                    ...
                    fi = open(".gitpy/" + str(repo.HEAD.commitId) + "/" + i, "rb")
                    string2 = fi.read()
            ...
            fo = open(specific, "r+")
            print("Now You see the Difference")
            string1 = fo.read()
            string1 = string1.splitlines()
